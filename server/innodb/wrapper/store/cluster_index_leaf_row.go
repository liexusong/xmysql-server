...
func NewClusterLeafRowHeader(frmMeta TableTuple) innodb.FieldDataHeader {
...
func NewClusterLeafRowHeaderWithContents(frmMeta TableTuple, content []byte) innodb.FieldDataHeader {
	var clr = new(ClusterLeafRowHeader)
	clr.FrmMeta = frmMeta
	clr.VarLengthContentMap = make(map[byte]uint16)
	//计算列长度
...
func NewClusterLeafRowData(meta *TableTupleMeta) innodb.FieldDataValue {
...
func NewClusterLeafRowDataWithContents(meta TableTuple, content []byte) innodb.FieldDataValue {
	var clusterLeafRowData = new(ClusterLeafRowData)
	clusterLeafRowData.Content = content
	clusterLeafRowData.RowValues = make([]innodb.Value, meta.GetColumnLength())
	clusterLeafRowData.meta = meta
	return clusterLeafRowData
}
...
func (cld *ClusterLeafRowData) ReadValue(index int) innodb.Value {
...
type ClusterLeafRow struct {
	...
	FrmMeta   TableTuple //表元祖属性
	RowValues []innodb.Value
}

func NewClusterLeafRow(content []byte, tableTuple TableTuple) innodb.Row {
	...
	currentRow.value = NewClusterLeafRowDataWithContents(tableTuple, content)
...
func (row *ClusterLeafRow) ReadValueByIndex(index int) innodb.Value {

	return row.value.ReadValue(index)
}

func NewClusterLeafRowWithFrm(meta *TableTupleMeta) innodb.Row {
	var rows = &ClusterLeafRow{
		header:  NewClusterLeafRowHeader(meta),
		value:   NewClusterLeafRowData(meta),
		FrmMeta: meta,
	}
	return rows
}
...
**/
//根据Row的主键值，或者是比较值做排序
func (row *ClusterLeafRow) Less(than innodb.Row) bool {
...
func (row *ClusterLeafRow) GetPrimaryKey() innodb.Value {
