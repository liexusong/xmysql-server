...
type FileSpaceHeader struct {
	...
	FreeLimit               []byte // 4 尚未被初始化的最小页号，大于或者等于这个页号的对应的XDES_ENTRY结构都没有被加入FreeList，当前尚未初始化的最小Page No。从该Page往后的都尚未加入到表空间的FREE LIST上。
...
func NewFileSpaceHeader(spaceId uint32) FileSpaceHeader {
	...
	fileSpaceHeader.NextUnusedSegmentId = []byte{0, 0, 0, 0, 0, 0, 0, 0}
