...
func (blockFile *BlockFile) ReadPageByNumber(pageNumber uint32) ([]byte, error) {
	...
	_, err := blockFile.StorageFile.Seek(int64(pageNumber)*16384, io.SeekStart)
	if err != nil {
		log.Fatal(err)
		return nil, err
	}
	b := make([]byte, 16384)
	_, err = blockFile.StorageFile.ReadAt(b, int64(pageNumber*(16384)))
	if err != nil {
		log.Fatal(err)
		return nil, err
	}
...
func (blockFile *BlockFile) ReadFileBySeekStartWithSize(offset uint64, size int64) []byte {
	blockFile.OpenFile()
	// 都是含前不含后的概念
	// offset是从0开始的, 可以比当前的文件内容长度大，多出的部分会用空(0)来代替
