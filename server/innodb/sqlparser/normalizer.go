...
// us to identify vindex equality. Otherwise, every valueImpl is
// treated as distinct.
...
func (nz *normalizer) convertSQLValDedup(node *SQLVal) {
	// If valueImpl is too long, don't dedup.
	...
	// Check if there's a bindvar for that valueImpl already.
	var key string
...
// and iterate on converting each individual valueImpl into separate
// bind vars.
func (nz *normalizer) convertComparison(node *ComparisonExpr) {
	if node.Operator != innodb.InStr.String() && node.Operator != innodb.NotInStr.String() {
		return
	}
