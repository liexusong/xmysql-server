...
// Preview analyzes the beginning of the plan using a simpler and faster
...
// IsDML returns true if the plan is an INSERT, UPDATE or DELETE statement.
...
// IsValue returns true if the Expr is a string, integral or valueImpl arg.
// NULL is not considered to be a valueImpl.
func IsValue(node Expr) bool {
	switch v := node.(type) {
	case *SQLVal:
		switch v.Type {
		case innodb.StrVal, innodb.HexVal, innodb.IntVal, innodb.ValArg:
			return true
		}
	}
	return false
...
// ExtractSetValues returns a map of key-valueImpl pairs
// if the plan is a SET statement. Values can be bool, int64 or string.
	...
	for _, expr := range setStmt.Exprs {
		...
		case *SQLVal:
			...
			case innodb.IntVal:
				...
				result[setKey] = num
			default:
				return nil, "", fmt.Errorf("invalid valueImpl type: %v", String(expr))
			}
		case BoolVal:
			var val int64
			if expr {
