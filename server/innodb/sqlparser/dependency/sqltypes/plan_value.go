...
// PlanValue represents a valueImpl or a list of values for
// a column that will later be resolved using bind vars and used
// to perform plan actions like generating the final plan or
// deciding on a route.
//
...
// of multi-valueImpl inserts), the representation is pivoted.
...
// ResolveValue resolves a PlanValue as a single valueImpl based on the supplied bindvars.
func (pv PlanValue) ResolveValue(bindVars map[string]*querypb.BindVariable) (Value, error) {
	...
	case pv.ListKey != "" || pv.Values != nil:
		...
		// multi-valueImpl constructs where a single valueImpl is expected.
		return NULL, errors.New("a list was supplied where a single valueImpl was expected")
	}
	return NULL, nil
}
...
func (pv PlanValue) lookupValue(bindVars map[string]*querypb.BindVariable) (*querypb.BindVariable, error) {
	...
	if bv.Type == querypb.Type_TUPLE {
		return nil, fmt.Errorf("TUPLE was supplied for single valueImpl bind var %s", pv.ListKey)
	}
	return bv, nil
}
...
func (pv PlanValue) ResolveList(bindVars map[string]*querypb.BindVariable) ([]Value, error) {
	...
	// This code is unreachable because the parser does not allow
	// single valueImpl constructs where multiple values are expected.
	return nil, errors.New("a single valueImpl was supplied where a list was expected")
}
...
func (pv PlanValue) lookupList(bindVars map[string]*querypb.BindVariable) (*querypb.BindVariable, error) {
	...
	if bv.Type != querypb.Type_TUPLE {
		return nil, fmt.Errorf("single valueImpl was supplied for TUPLE bind var %s", pv.ListKey)
	}
	return bv, nil
}
...
func rowCount(pvs []PlanValue, bindVars map[string]*querypb.BindVariable) (int, error) {
	...
	if count == -1 {
		...
		// at least one valueImpl (enforced by vtgate), and AST lists
		// must have at least one valueImpl (enforced by the parser).
		// Also lists created internally after vtgate validation
		// ensure at least one valueImpl.
...
func ResolveRows(pvs []PlanValue, bindVars map[string]*querypb.BindVariable) ([][]Value, error) {
	...
	for j, pv := range pvs {
		...
		case pv.Values != nil:
			...
			// default case is a NULL valueImpl, which the row values are already initialized to.
		}
	}
	return rows, nil
}
