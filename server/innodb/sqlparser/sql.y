...
| SHOW show_session_or_global VARIABLES like_or_where_opt
  {
    $$ = &Show{Scope: $2, Type: string($3) ,ShowValues: $4}
  }
| SHOW VINDEXES
  {
    $$ = &Show{Type: string($2)}
...
other_statement:
  DESC table_name
  {
    $$ = &OtherRead{Type:string($1),TableName:$2}
  }
| DESC table_name column_name
 {
    $$ = &OtherRead{Type:string($1),TableName:$2,ColName:$3 }
  }
|  DESCRIBE table_name
  {
    $$ = &OtherRead{Type:string($1),TableName:$2}
...
| value_expression IN col_tuple
  {
    $$ = &ComparisonExpr{Left: $1, Operator: string(innodb.InStr), Right: $3}
  }
| value_expression NOT IN col_tuple
  {
    $$ = &ComparisonExpr{Left: $1, Operator: string(innodb.NotInStr), Right: $4}
  }
| value_expression LIKE value_expression like_escape_opt
  {
    $$ = &ComparisonExpr{Left: $1, Operator: string(innodb.LikeStr), Right: $3, Escape: $4}
  }
| value_expression NOT LIKE value_expression like_escape_opt
  {
    $$ = &ComparisonExpr{Left: $1, Operator: string(innodb.NotLikeStr), Right: $4, Escape: $5}
  }
| value_expression REGEXP value_expression
  {
    $$ = &ComparisonExpr{Left: $1, Operator: innodb.RegexpStr.String(), Right: $3}
  }
| value_expression NOT REGEXP value_expression
  {
    $$ = &ComparisonExpr{Left: $1, Operator: innodb.NotRegexpStr.String(), Right: $4}
  }
...
compare:
  '='
  {
    $$ = innodb.EqualStr.String()
  }
| '<'
  {
    $$ = innodb.LessThanStr.String()
  }
| '>'
  {
    $$ = innodb.GreaterThanStr.String()
  }
| LE
  {
    $$ = innodb.LessEqualStr.String()
  }
| GE
  {
    $$ = innodb.GreaterEqualStr.String()
  }
| NE
  {
    $$ = innodb.NotEqualStr.String()
  }
| NULL_SAFE_EQUAL
  {
    $$ = innodb.NullSafeEqualStr.String()
  }
...
| value_expression '&' value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.BitAndStr), Right: $3}
  }
| value_expression '|' value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.BitOrStr), Right: $3}
  }
| value_expression '^' value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.BitXorStr), Right: $3}
  }
| value_expression '+' value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.PlusStr), Right: $3}
  }
| value_expression '-' value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.MinusStr), Right: $3}
  }
| value_expression '*' value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.MultStr), Right: $3}
  }
| value_expression '/' value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.DivStr), Right: $3}
  }
| value_expression DIV value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.IntDivStr), Right: $3}
  }
| value_expression '%' value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.ModStr), Right: $3}
  }
...
| value_expression SHIFT_LEFT value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.ShiftLeftStr), Right: $3}
  }
| value_expression SHIFT_RIGHT value_expression
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.ShiftRightStr), Right: $3}
  }
| column_name JSON_EXTRACT_OP value
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.JSONExtractOp), Right: $3}
  }
| column_name JSON_UNQUOTE_EXTRACT_OP value
  {
    $$ = &BinaryExpr{Left: $1, Operator: string(innodb.JSONUnquoteExtractOp), Right: $3}
  }
...
| BINARY value_expression %prec UNARY
  {
    $$ = &UnaryExpr{Operator: string(innodb.BinaryStr), Expr: $2}
  }
| UNDERSCORE_BINARY value_expression %prec UNARY
  {
    $$ = &UnaryExpr{Operator: string(innodb.UBinaryStr), Expr: $2}
  }
| '+'  value_expression %prec UNARY
  {
    if num, ok := $2.(*SQLVal); ok && num.Type == innodb.IntVal {
      $$ = num
    } else {
      $$ = &UnaryExpr{Operator: string(innodb.UPlusStr), Expr: $2}
    }
  }
| '-'  value_expression %prec UNARY
  {
    if num, ok := $2.(*SQLVal); ok && num.Type == innodb.IntVal {
      ...
      $$ = &UnaryExpr{Operator: string(innodb.UMinusStr), Expr: $2}
    }
  }
| '~'  value_expression
  {
    $$ = &UnaryExpr{Operator: string(innodb.TildaStr), Expr: $2}
  }
| '!' value_expression %prec UNARY
  {
    $$ = &UnaryExpr{Operator: string(innodb.BangStr), Expr: $2}
  }
...
| CHAR length_opt charset_opt
  {
    $$ = &ConvertType{Type: string($1), Length: $2, Charset: $3, Operator: string(innodb.CharacterSetStr)}
  }
| CHAR length_opt ID
  {
...
| MOD
| NATURAL
| NEXT // next should be doable as non-reserved, but is not due to the special `select next num_val` plan that vitess supports
| NOT
| NULL
| ON
| OR
...
/*
  ...
  when we rewrite the plan, so no issue should arise.
